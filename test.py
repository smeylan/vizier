...
def check_return_data(return_json, expectedOutput):
...
class FlaskRouteTester():
  ...
  def test(self, method='GET'):        
    ...
    if (self.output is not None):
      ...
      check_return_data(return_json, self.output)
      return(return_json)
...
def test_routes():
  ...
  userToDelete = {'vizierUserId':rv['vizierUserId']}

  t = FlaskRouteTester('/removeUser', userToDelete, {"success":1})  
  t.test()
